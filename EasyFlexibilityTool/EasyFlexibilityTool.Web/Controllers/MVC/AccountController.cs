namespace EasyFlexibilityTool.Web.Controllers.MVC
{
    ...
    using Services;
    using ViewModels;
    using EasyFlexibilityTool.Web.Controllers.WebAPI;
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    {
                        // Use the user.UserName here because we use the Email as the login and UserName is a user-friendly name
                    ...
                    {
                        ModelState.AddModelError("", "Please confirm the email first");
                    }
                }
                else
                {
        ...
        {
            RegisterViewModel model = new RegisterViewModel { Email = email };
            return View(model);
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //Disable automatic signing in during registration
                    ...
                    var messageBody = Engine.Razor.RunCompile("EmailConfirmationMessage", typeof(string), callbackUrl);

                    var emailTemplate = DbContext.EmailTemplates.Where(et => et.TemplateName == "Confirm Account").FirstOrDefault();
                    if (emailTemplate.TemplateContent != null)
                    {
                        messageBody = emailTemplate.TemplateContent.Replace("{ConfirmAccountLink}", string.Format(@"<a href='{0}'>here</a>", callbackUrl));
                    }

                    await UserManager.SendEmailAsync(user.Id, "Confirm your email", messageBody);

                    ViewBag.Message = $"We have sent you the verification message to {model.Email}. Please check your email";
                    model.Email = null;
                    model.Password = null;
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var messageBody = Engine.Razor.RunCompile("ForgotPasswordMessage", typeof(string), callbackUrl);
